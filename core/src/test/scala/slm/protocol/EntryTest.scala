...
class EntryTest extends FunSuite:
  ...
  def encodeAndDecodeObject[A: Encoder: Decoder](
      value: A,
      expectedJson: Json
  ): A =
  ...
  test(
    "Tty with password method should encode to and decode from JSON correctly"
  ) {
  ...
  test("AuthMethod should encode to and decode from JSON correctly") {
    ...
    val decoded =
      decode[AuthMethod](encodedString).getOrElse(fail("Failed to decode JSON"))
  ...
  test("Entry should handle polymorphic encoding/decoding") {
    ...
    val entries: List[Entry] = List(
      ...
      Entry.Gateway(
        "Nested Gateway",
        List(Entry.InfoPage("Nested Info", infoUri))
      )
    )
